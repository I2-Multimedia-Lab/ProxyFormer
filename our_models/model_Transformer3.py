...
class PCTransformer(nn.Module):
    ...
    """
    def __init__(self, in_chans=3, embed_dim=768, depth=[6, 6], num_heads=8, mlp_ratio=2., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                        num_query = 128, knn_layer = -1):
        super().__init__()
        ...
        self.feat_extractor_teacher = FeatureExtractor(out_dim=512, n_knn=20)
        ...
        self.input_proj_teacher = nn.Sequential(
            nn.Conv1d(256, embed_dim, 1),
            nn.BatchNorm1d(embed_dim),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv1d(embed_dim, embed_dim, 1)
        ...
        self.gen_missing_feature = nn.Sequential(
            nn.Conv1d(embed_dim, 768, 1),
            nn.BatchNorm1d(768),
            nn.ReLU(),
            nn.Conv1d(768, 768, 1),
            nn.BatchNorm1d(768),
            nn.ReLU(),
            nn.Conv1d(768, self.miss_f_out, 1)
        )
    ...
    def _init_teacher(self):  
        ...
        for param_encoder, param_teacher in zip(self.feat_extractor.parameters(), self.feat_extractor_teacher.parameters()):
            param_teacher.detach()
            param_teacher.data.copy_(param_encoder.data)
            param_teacher.requires_grad = False
        for param_encoder, param_teacher in zip(self.pos_embed.parameters(), self.pos_embed_teacher.parameters()):
            param_teacher.detach()
            param_teacher.data.copy_(param_encoder.data)
            param_teacher.requires_grad = False
        for param_encoder, param_teacher in zip(self.input_proj.parameters(), self.input_proj_teacher.parameters()):
            param_teacher.detach()
            param_teacher.data.copy_(param_encoder.data)
            param_teacher.requires_grad = False
    ...
    def forward(self, inpc, missing_part=None):
        ...
        # Query Generator
        global_feature, fps_points, incomplete_point_feature = self.feat_extractor(inpc.transpose(1,2).contiguous()) # gfv: B 512 1, p: B 3 128, ipf: B 256 128
        global_feature = global_feature.squeeze() # gfv: B 512

        coarse_point_cloud = self.coarse_gen(global_feature).reshape(bs, -1, 3) # missing_coarse: B 128 3
        ...
        pos =  self.pos_embed(fps_points).transpose(1,2)
        x = self.input_proj(incomplete_point_feature).transpose(1,2)  # x: B 128 384

        num_pre_proxy = self.num_query # 128
        feature_dim = x.size(2) # 384
        # print("x: ", x.size())
            
        # Here Got Some Question!!!
        # using imcomplete point cloud's feature to predict missing part's feature B M(224) C(384)
        ...
        x = x + pos # B 128 384
        y = missing_feature + random_pos_embedding # B num_pre_proxy 384

        # print("en_input: ", encoder_input.size())

        # encoder  pre_missing_part_feature: B num_pre_proxy 384
        for i, blk in enumerate(self.encoder):
            pre_missing_part_feature = blk(x, y)
        ...
        if missing_part is None:
            return pre_missing_part_feature, coarse_point_cloud
        else:
            _, miss_fps_points, true_missing_part_feature = self.feat_extractor_teacher(missing_part.transpose(1,2).contiguous())
            true_missing_part_feature = self.input_proj_teacher(true_missing_part_feature).transpose(1,2)
            miss_pos = self.pos_embed_teacher(miss_fps_points).transpose(1,2)
            true_missing_part_feature = true_missing_part_feature + miss_pos
            return pre_missing_part_feature, coarse_point_cloud, true_missing_part_feature
