...
from our_models.Transformer_Coarse import PCTransformer
...
class PoinTr_Encoder(nn.Module):
    ...
    def get_feature_loss(self, miss_f, pre_miss_f):
        loss_f = self.loss_f(pre_miss_f, miss_f)
        return loss_f

    def forward(self, xyz, missing_part=None):
        if missing_part is None:
            y, coarse_point_cloud = self.base_model(xyz) # B M C and B M 3
        
            B, M, _ = coarse_point_cloud.shape

            global_feature = self.increase_dim(y.transpose(1,2)).transpose(1,2) # B M 1024
            ...
            x = self.base_model(xyz, missing_part)
            return x
